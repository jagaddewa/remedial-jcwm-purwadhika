    ...
    return (dispatch) => {
        Axios.get(`http://localhost:4000/users/${id}`)
            .then(res => {
                ...
                Axios.patch(`http://localhost:4000/users/${id}`, { cart: tempCart })
                    .then(res => {
                        Axios.get(`http://localhost:4000/users/${id}`)
                            .then(res => {
                                return dispatch({
...
export const delCart = (idUser, idProdCart) => {
    return (dispatch) => {
        Axios.get(`http://localhost:4000/users/${idUser}`)
            .then(res => {
                let tempCart = res.data.cart
                tempCart.splice(idProdCart, 1)

                Axios.patch(`http://localhost:4000/users/${idUser}`, { cart: tempCart })
                    .then(res => {
                        Axios.get(`http://localhost:4000/users/${idUser}`)
                            .then(res => {
                                return dispatch({
...
export const checkout = (idUser, dataTrans) => {
    return (dispatch) => {
        ...
        Axios.post('http://localhost:4000/history', dataTrans)
            .then(res => {
                let idUser = localStorage.getItem('idUser')
                
                Axios.get(`http://localhost:4000/history?idUser=${idUser}`)
                    .then(res => {
                        return dispatch({
                            type: 'GET_HISTORY',
                            payload: res.data
            ...
            .then(res => {
                ...
                Axios.patch(`http://localhost:4000/users/${idUser}`, { cart: [] })
                    .then(res => {
...
export const getHistory = () => {
    return (dispatch) => {
        let idUser = localStorage.getItem('idUser')
        Axios.get(`http://localhost:4000history?idUser=${idUser}`)
            .then(res => {
                return dispatch({
                    type: 'GET_HISTORY',
                    payload: res.data
